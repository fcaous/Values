...
const GROQ_API_KEY   = process.env.GROQ_API_KEY || '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';
  ...
  return new Promise((resolve, reject) => {
    ...
    const opts = {
      ...
      headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`,
        'Content-Type': 'application/json', 'Prefer': 'return=representation', ...(extraHeaders || {}) },
    };
...
app.post('/api/admin/pets/:id/history', authAdmin, async (req, res) => {
  try {
    ...
    const point = { pet_id: req.params.id, token_value: String(token_value).trim(),
...
app.post('/api/ai/chat', async (req, res) => {
  ...
  try {
    ...
    const groqReq = https.request({
      ...
      headers: { 'Authorization': `Bearer ${GROQ_API_KEY}`, 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(groqBody) },
    }, groqRes => {
      let buf = '';
